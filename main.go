...
func (self CheckSystemdStatus) Get(name string) (Status) {
  ...
  units, err := conn.ListUnits() ; if err != nil {
    newStatus.Code = CRIT
...
func NewChecker(path string) (*Checker, error) {
   ...
   files, err := filepath.Glob(mask) ; if err != nil {
     return nil, err
   }
   ...
   for _, filename := range files {
     ...
     sections, err := unit.Deserialize(bytes.NewReader(content)) ; if err != nil {
       return nil, err
     }
...
func main() {
  ...
  checker, err := NewChecker("system") ; if err != nil {
    panic(err)
  }
